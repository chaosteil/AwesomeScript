...
void Tokenizer::_checkUnexpectedEnd() throw(Exception){
	if(_isFinished()){
		throw Exception(Exception::ParsingError, "Unexpected end of source.");
	}
}
...
void Tokenizer::_invalidCharacter() throw(Exception){
	...
	if(_tokenValueBuffer.str().length() == 0){
		errormessage << "Invalid character '" << _currentChar << "'";
		throw Exception(Exception::SyntaxError, errormessage.str());
	}else{
		// We write what's in the buffer and our invalid char
		errormessage << "Invalid character '" << _currentChar <<
			"' after " << _tokenValueBuffer.str();
		throw Exception(Exception::SyntaxError, errormessage.str());
	}
}
